        ...
        try:
            for i in range(num_images):
                ...
                if min_w != 1.0 or max_w != 1.0 or min_h != 1.0 or max_h != 1.0:
                    ...
                    image = np.where(image >= 0.05, image,
                                     np.zeros_like(image))
                    image = crop_non_empty(image)
...
def write_to_pickle(filename, images, indices, positions, boxes, labels, digits):
    num = images.shape[0]
    rows, cols = images[0].shape
    dict = {"height": np.repeat(rows,num),
            "width": np.repeat(cols,num),
            "digits": digits,
            "indices":indices,
            "positions":positions,
            "boxes":boxes,
        ...
        example = tf.train.Example(
            features=tf.train.Features(
                feature={
                    ...
                    "boxes":
                    _bytes_feature(np.asarray(
                        boxes[index], dtype=np.int32).tostring()),
                    "labels":
                    _bytes_feature(
    ...
    for string_record in record_iterator:
        ...
        images_list.append(
            np.fromstring(
                ...
                dtype=np.float32))
        digits_list.append(
            int(example.features.feature["digits"].int64_list.value[0]))
...
if __name__ == "__main__":
    ...
    DEFAULT_IMAGES_PER_DIGIT = 25000
    DEFAULT_TEST_SET_SIZE = 5000

    def CONSTRAINS(x, y): return True
    NAME_SURFIX = ""
    ...
    for num_digits in range(0, args.max_digits + 1):
        ...
        if SAVER_TFRECORDS is True:
            print("Writing {} digit images... ".format(
                num_digits), end="", flush=True)
            write_to_records(
        ...
        if num_digits == max(NUM_IN_COMMON):
            ...
            print(
                "Writing 0-{} digit images to common file... ".format(
                    name_of_common),
                end="",
                flush=True,
            )
            ...
            write_to_pickle(
                MULTI_MNIST_FOLDER + "common" + name_of_common+".pkl",
                common_images[args.test_set_size:],
                common_indices[args.test_set_size:],
                common_positions[args.test_set_size:],
                common_boxes[args.test_set_size:],
            ...
            print(
                "Writing 0-{} digit images to test file... ".format(
                    name_of_common),
                end="",
                flush=True,
            )
            ...
            write_to_pickle(
                MULTI_MNIST_FOLDER + "test" + name_of_common + ".pkl",
                common_images[:args.test_set_size],
                common_indices[:args.test_set_size],
                common_positions[:args.test_set_size],
                common_boxes[:args.test_set_size],
